...
#import "VKPhotoManager.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import <Photos/Photos.h>
#import "VKAlbumModel.h"
#import "VKPhotoModel.h"

static inline ZCImageType vk_ImageTypeFromData(NSData *imageData)
{
...
-(void)vk_RequestAuthorization:(void (^)(BOOL result, BOOL alert ,NSArray * allAlbum))authorizationCompletion{
    ...
    if (iOS8Later) {
       ...
       __block NSMutableArray *albums = [[NSMutableArray alloc] init];
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            ...
            switch (status) {
                ...
                case PHAuthorizationStatusAuthorized: {
                    ...
                    albums = [self vk_IOS8LaterGetAllAlbum];
        ...
        [self vk_BeforeIOS8GetAllAlbum:failureBlock albums:^(NSArray *allAlbum) {
 ...
 *  @return
 */
-(NSMutableArray *)vk_IOS8LaterGetAllAlbum {
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if ([collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"相机胶卷"]) {
            VKAlbumModel *albumM = [self vk_transformToAlbumModel:asset albumName:collection.localizedTitle];
            if (albumM) {
...
- (void)vk_BeforeIOS8GetAllAlbum:(ALAssetsLibraryAccessFailureBlock)failureBlock
                          albums:(void (^)(NSArray * allAlbum))completion{
    ...
    ALAssetsLibraryGroupsEnumerationResultsBlock resultsBlock = ^(ALAssetsGroup *group, BOOL *stop) {
        ...
        if (group && [group numberOfAssets] > 0) {
            ...
            if ([name isEqualToString:@"Camera Roll"] || [name isEqualToString:@"相机胶卷"]) {
                VKAlbumModel *albumM = [self vk_transformToAlbumModel:group albumName:name];
                if (albumM) {
                    [albums insertObject:albumM atIndex:0];
                }
...
- (void)vk_GetAllAlbum:(void (^)(NSArray * allAlbum))completion {
    if (iOS8Later) {
        ...
        [self vk_BeforeIOS8GetAllAlbum:nil albums:^(NSArray *allAlbum) {
            if (completion) {
                completion(allAlbum);
            }
 ...
 *  @return AlbumModel
 */
- (VKAlbumModel *)vk_transformToAlbumModel:(id)result albumName:(NSString *)albumName
{
    

    NSArray *photos = [self vk_getAllPhotosWithAlbum:result];
    
    if (photos.count < 1) {
        
        return nil;
    }
    VKAlbumModel *albumM = [[VKAlbumModel alloc] init];
    albumM.albumName = [self vk_GetNewAlbumName:albumName];
 ...
 *  @return
 */
- (NSString *)vk_GetNewAlbumName:(NSString *)name {
    NSString *newName;
...
- (NSArray *)vk_getAllPhotosWithAlbum:(id)album
{
