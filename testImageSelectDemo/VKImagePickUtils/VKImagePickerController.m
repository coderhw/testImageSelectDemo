...
@interface VKImagePickerController ()

@end

@implementation VKImagePickerController
...
- (instancetype)initWithRootViewControllerWithPredicate:(NSPredicate *)predicate singleSelect:(BOOL)singleSelect {
    ...
    VKPhotoManager *photoManager = [[VKPhotoManager alloc] init];
    VKAlbumInfoViewController *albumInfoVC = [[VKAlbumInfoViewController alloc] init];
...
-(NSMutableArray *)siftAlbums:(NSArray *)albums predicate:(NSPredicate *)predicate {
    ...
    if (predicate) {
        ...
        [albums enumerateObjectsUsingBlock:^(VKAlbumModel *  _Nonnull albumM, NSUInteger idx, BOOL * _Nonnull stop) {
...
-(void)pushPhotoViewController:(VKAlbumInfoViewController *)albumInfoVC
                  photoManager:(VKPhotoManager *)photoManager
                  predicate:(NSPredicate *)predicate
                  singleSelect:(BOOL)singleSelect {
    
    if([photoManager vk_AuthorizationStatusAuthorized]) {
        
        [photoManager vk_GetAllAlbum:^(NSArray *allAlbum) {
        ...
        photosVC.singleSelect = singleSelect;
        [self pushViewController:photosVC animated:YES];
        [photoManager vk_RequestAuthorization:^(BOOL result ,BOOL alert,NSArray *albums) {
...
-(void)imagePickerDidFinish:(NSNotification *)objc {
    ...
    if (images.count > 0) {
        [self.pickerDelegate vk_ImagePickerDidFinishPickingImage:images];
    }
}
